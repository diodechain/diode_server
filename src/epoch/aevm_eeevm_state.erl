...
% -ifdef(COMMON_TEST).
-define(TEST_LOG(Format, Data), io:format(["TEST: " | Format], Data)).
-define(DEBUG_LOG(Format, Data), io:format(["DEBUG: " | Format], Data)).
% -else.
% -define(TEST_LOG(Format, Data), ok).
% -define(DEBUG_LOG(Format, Data), ok).
% -endif.
...
call_contract(Caller, Target, CodeAcc, CallGas, Value, Data, State) ->
    case vm_version(State) of
        ...
        _ ->
            ...
            TargetKey  = <<Target:256>>,
            CodeAccKey = <<CodeAcc:256>>,
            ChainAPI   = chain_api(State),
            {ok, State1} = save_store(State),
            ChainState = chain_state(State1),
            Origin     = <<(origin(State)):256>>,
            {Res, ChainState1} =
                ChainAPI:call_contract(TargetKey, CodeAccKey, CallGas, Value, Data,
                                       CallStack, Origin, ChainState),
            State2 = aevm_eeevm_store:init(ChainAPI:store(ChainState1), State),
            State3 = set_chain_state(ChainState1, State2),
            GasSpent = ChainAPI:gas_spent(Res),
            {Tag, Return} = ChainAPI:return_value(Res),
            State4 = set_return_data(Return, State3),
            {Tag, Return, GasSpent, State4}
    end.
...
set_refund(Value, State)  -> maps:put(refund, Value, State).
...
set_return_data(Value, State) -> maps:put(return_data, Value, State).
