...
for i <- 1..3 do
  time_ms = :timer.tc(fn ->
    for _ <- 1..100 do
      tree = HeapMerkleTree.insert_items(HeapMerkleTree.new(), test_data)

      if HeapMerkleTree.root_hash(tree) != ref do
        raise("Error #{Base16.encode(HeapMerkleTree.root_hash(tree))} != #{Base16.encode(ref)}")
      end
    end
  end)
  |> elem(0)
